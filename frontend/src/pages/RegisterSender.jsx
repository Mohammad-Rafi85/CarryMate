    ...
    return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 relative overflow-hidden">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-6xl grid lg:grid-cols-5 bg-white rounded-[40px] shadow-2xl overflow-hidden border border-slate-100">
                ...
                {/* Left Side - Info */}
                <div className="hidden lg:flex lg:col-span-2 bg-indigo-900 p-16 flex-col justify-between relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <div className="flex items-center gap-3 mb-16">
                            <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-xl">
                                <Building className="text-indigo-900" size={20} />
                            </div>
                            <span className="text-xl font-bold tracking-tight text-white">Sender Fast-Track</span>
                        </div>
                        <h2 className="text-4xl font-bold text-white mb-8 leading-tight">Ship in under 60 seconds.</h2>
                        <div className="space-y-8">
                            {[
                                { icon: <Shield size={18} />, title: 'Fully Insured', desc: 'Enterprise-grade escrow protects your funds until delivery.' },
                                { icon: <MapPin size={18} />, title: 'Real-time Updates', desc: 'Track your packages globally with live status tracking.' },
                                { icon: <Zap size={18} />, title: 'Instant Matching', desc: 'Connect with verified AP travelers immediately.' }
                            ].map((item, i) => (
                                <div key={i} className="flex gap-4">
                                    <div className="mt-1 w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center text-indigo-200 shrink-0">
                                        {item.icon}
                                    </div>
                                    <div>
                                        <h4 className="text-white font-bold mb-1">{item.title}</h4>
                                        <p className="text-indigo-200 text-sm font-medium leading-relaxed">{item.desc}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
