...
const Dashboard = () => {
    ...
    const fetchDeliveriesOnly = async () => {
        try {
            const res = await api.get('/deliveries/my-requests');
    ...
    const getStatusStyles = (status) => {
        switch (status) {
            ...
            case 'MATCHED': return 'bg-sky-50 text-sky-600 border-sky-100';
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-10 mb-16">
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            <span className="px-3 py-1 bg-sky-50 text-sky-600 text-[10px] font-black uppercase tracking-widest rounded-full border border-sky-100">Live Network</span>
                        </div>
                    ...
                    <div className="flex items-center gap-4">
                        <button onClick={() => setIsDeliveryModalOpen(true)} className="btn-primary py-4 px-8">
                    ...
                    <div className="lg:col-span-2 space-y-12">
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-8 px-2">
                                ...
                                <button className="text-sm font-bold text-sky-600 hover:text-sky-800 transition-colors">View History</button>
                            </div>
                                ...
                                {deliveries.length === 0 ? (
                                    ...
                                    deliveries.map((delivery, i) => (
                                        <motion.div
                                            ...
                                            className="bg-white border border-slate-200 rounded-3xl p-8 hover:border-sky-200 transition-all shadow-sm hover:shadow-md group"
                                        >
                                            <div className="flex justify-between items-start mb-6">
                                                <div>
                                                    <h3 className="font-bold text-slate-950 text-xl group-hover:text-sky-600 transition-colors">{delivery.itemDescription}</h3>
                                            ...
                                            <div className="flex flex-col md:flex-row md:items-center gap-6 p-6 bg-slate-50 rounded-2xl mb-6">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center border border-slate-200 text-sky-600"><MapPin size={16} /></div>
                                            ...
                                            <div className="flex justify-between items-center">
                                                ...
                                                <button className="text-sm font-bold text-sky-600 flex items-center gap-2 hover:gap-3 transition-all">Details <ArrowRight size={16} /></button>
                                            </div>
                                        </motion.div>
                                    ))
                                )}
                            ...
                            <div className="space-y-6">
                                {trips.length === 0 ? (
                                    <div className="p-10 text-center bg-white border border-slate-200 rounded-3xl">
                                        ...
                                        <Link to="/post-trip" className="text-sky-600 font-bold block text-sm underline decoration-2 underline-offset-4">List your journey</Link>
                                    </div>
                                ) : (
                                    trips.map((trip, i) => (
                                        <div key={trip.id || i} className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm hover:border-sky-100 transition-all">
                                            <div className="flex items-center gap-4 mb-6">
                                                <div className="w-10 h-10 bg-sky-50 text-sky-600 rounded-xl flex items-center justify-center"><Calendar size={20} /></div>
                                            ...
                                            <div className="pt-4 border-t border-slate-50 flex justify-between items-center">
                                                ...
                                                <span className="text-sky-600 font-bold">{trip.availableCapacityKg} kg</span>
                                            </div>
                                        </div>
                                    ))
                                )}
